  ...
  return (
    <section id="pricing" className="py-16 sm:py-20 px-4 sm:px-6 relative z-10">
      <div className="max-w-6xl mx-auto">
        <motion.div {...iv(0)} className="text-center mb-10 sm:mb-12">
          <p className="text-xs font-bold tracking-[0.22em] uppercase mb-3" style={{ color:"#fbbf24" }}>Pricing</p>
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight" style={{ color:"var(--text-white)" }}>
            Simple, token-based pricing
          </h2>
          <p className="mt-3 text-sm sm:text-base max-w-md mx-auto" style={{ color:"var(--text-secondary)" }}>
            Pay for what you use. Tokens power every response — more tokens, more conversations.
          </p>

          {/* Token explain */}
          <div className="inline-flex flex-wrap justify-center gap-3 mt-5 px-4 sm:px-6 py-3 rounded-xl"
            style={{ background:`rgba(${G2},0.05)`, border:`1px solid rgba(${G2},0.12)` }}>
            <span className="text-xs font-medium" style={{ color:"var(--text-muted)" }}>💡 1 token ≈ 4 chars</span>
            {TOKEN_INFO.examples.map(e => (
              <span key={e.action} className="text-xs" style={{ color:"var(--text-secondary)" }}>
                <span style={{ color:"var(--text-white)", fontWeight:600 }}>{e.tokens}</span> {e.action}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mt-6">
            <span className="text-xs sm:text-sm font-medium" style={{ color: annual ? "var(--text-muted)" : "var(--text-white)" }}>Monthly</span>
            <button
            ...
            <span className="text-xs sm:text-sm font-medium" style={{ color: annual ? "var(--text-white)" : "var(--text-muted)" }}>
              Annual <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background:`rgba(${G2},0.15)`, color:"#4ade80" }}>-20%</span>
            </span>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-5">
          {PRICING.map((plan, i) => {
            ...
            const displayPrice = rawPrice === 0 ? "₹0" : annual ? `₹${Math.floor(rawPrice * 0.8)}` : plan.price;

            return (
              <motion.div key={plan.name} {...iv(i * 0.08)}
                className="relative rounded-2xl p-5 sm:p-6 flex flex-col overflow-hidden"
                style={{
                  background: plan.highlight ? `rgba(${G2},0.07)` : "var(--navy-card)",
                  border: plan.highlight ? `2px solid ${plan.accent}` : `1px solid ${plan.accentBorder}`,
                }}
                whileHover={{ y:-4, boxShadow:`0 12px 44px rgba(0,0,0,0.6)` }} transition={bs}>
                ...
                <div className="mb-5">
                  ...
                  <div className="flex items-baseline gap-1 mt-3">
                    <span className="text-3xl sm:text-4xl font-extrabold" style={{ color:"var(--text-white)" }}>{displayPrice}</span>
                  ...
                  <p className="text-xs mt-1.5" style={{ color:"var(--text-muted)" }}>{plan.tagline}</p>
                </div>
                ...
                <ul className="space-y-2.5 flex-1 mb-6">
                  {plan.features.map(f => (
                    <li key={f.text} className="flex items-start gap-2.5">
                      <span className="flex-shrink-0 text-xs mt-0.5" style={{ color: f.ok ? "#4ade80" : "var(--text-faint)" }}>
                        {f.ok ? "✓" : "✕"}
                      </span>
                      <span className="text-xs leading-relaxed" style={{ color: f.ok ? "var(--text-secondary)" : "var(--text-faint)" }}>
                        {f.text}
                      </span>
                    </li>
                  ))}
                ...
                <motion.button
                  ...
                  style={plan.highlight
                    ...
                    : { background: plan.accentBg, color: plan.accent, border:`1px solid ${plan.accentBorder}` }}
                  whileHover={{ filter:"brightness(1.1)" }} whileTap={{ scale:0.97 }} transition={bs}>
                  {plan.cta}
                </motion.button>
              </motion.div>
            );
        ...
        {/* Token refill note */}
        <motion.p {...iv(0.3)} className="text-center text-xs mt-6" style={{ color:"var(--text-muted)" }}>
          Tokens reset every month. Unused tokens don't carry over. Need more? Top-up packs coming soon.
        </motion.p>
      </div>
    </section>
  );
